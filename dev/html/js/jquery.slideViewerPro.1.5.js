...
jQuery.fn.slideViewerPro = function(settings) {
	...
	return this.each(function(){
		...
		*/
		var pictWidth = 263;//container.find("img").width(); 
		var pictHeight = 230;//container.find("img").height();
		var pictEls = container.find("li").size();
		(pictEls >= settings.thumbs) ? null : settings.thumbs = pictEls;
		var slideViewerWidth = pictWidth*pictEls;
		var thumbsWidth = 72;//Math.round(pictWidth*settings.thumbsPercentReduction/100);
		var thumbsHeight = 78;//Math.round(pictHeight*settings.thumbsPercentReduction/100);
		var pos = 0;
		var r_enabled = true;
		var l_enabled = true;
