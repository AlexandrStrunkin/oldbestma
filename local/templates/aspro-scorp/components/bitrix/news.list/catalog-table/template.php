					...
					<div class="item<?=($bShowImage ? '' : ' wti')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
						<?if($bShowImage):?>
							<div class="image">
								...
								<?elseif($imageDetailSrc):?><a href="<?=$imageDetailSrc?>" alt="<?=($bImage ? $arItem['FIELDS']['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['FIELDS']['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="img-inside fancybox">
								<?endif;?>
						...
						<div class="text">
							...
							<?if(strlen($arItem['DISPLAY_PROPERTIES']['PRICE']['VALUE']) || $bOrderButton):?>
								<div class="row foot">
									<div class="<?=(!$bOrderButton ? 'col-md-12 col-sm-12 col-xs-12' : 'col-md-6 col-sm-7 col-xs-12')?> pull-left">
										<?// element price?>
