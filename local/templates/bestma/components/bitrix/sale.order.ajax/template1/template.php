        ...
        if ($arResult["USER_VALS"]["CONFIRM_ORDER"] == "Y" || $arResult["NEED_REDIRECT"] == "Y") {
            ...
            if ($_POST["is_ajax_post"] != "Y") {
                ...
                <?= bitrix_sessid_post() ?>
                    ...
                    if (count($arResult["PERSON_TYPE"]) > 1) {
                        ...
                        <div class="content_check ">
                            ...
                            <ul class="shaiser_content">
                                <li>
                        <div class="w_content">
                            <table class="sale_order_full_table type_person">
                                <tr>
                                    <td>
                                        <?
                                        foreach ($arResult["PERSON_TYPE"] as $v) {
                                            ?><div><span style="display: table-cell; vertical-align: middle; padding-right: 5px;"><input type="radio" id="PERSON_TYPE_<?= $v["ID"] ?>" name="PERSON_TYPE" value="<?= $v["ID"] ?>"<? if ($v["CHECKED"] == "Y") echo " checked=\"checked\""; ?> onClick="submitForm()"></span> <span style="display: table-cell; vertical-align: middle;"><label for="PERSON_TYPE_<?= $v["ID"] ?>"><?= $v["NAME"] ?></label></span></div><?
                                        }
                                        ?>
                    ...
                    <?if($arResult["ORDER_PRICE"] < 10000){?>
                    <div class="w_content">    
                        <input style="color:#757d8a; text-decoration:none;" class="gray_button" type="button" name="" value="<?= GetMessage("SOA_TEMPL_BUTTON") ?>">
                    ...
                    <?}else{?>
                    <div class="w_content">    
                        <input style="" class="gray_button" type="button" name="submitbutton" onClick="submitForm('Y');" value="<?= GetMessage("SOA_TEMPL_BUTTON") ?>">
                        <div class="clear"></div>
                    </div>
                    <?}?>
