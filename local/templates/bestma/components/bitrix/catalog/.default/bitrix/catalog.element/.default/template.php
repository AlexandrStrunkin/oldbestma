...
<div class="product_wrapp prod_parent_block_8dfd71c72ba4339630bf25e7445cf49a">
    ...
    <div class="slider_and_prices">
        ...
        <?      $date_second_add = strtotime($arResult["DATE_CREATE"]); # Выразили нужную вам дату в секундах
                $current_date = mktime(date('d.m.Y G:i:s')); 
                $date_second_1 = $date_second_add + 1209600; //1209600; # прибавить 30 дней (выраженные в секундах) 
                $date_second = date('d.m.Y G:i:s', $date_second_1);
        ?> 
            <?
            if($current_date < $date_second_1){ ?>
            ...
            <? if ((is_null($arResult["MORE_PHOTO"]) || (count($arResult["MORE_PHOTO"]) == 0))): ?>
                ...
                <div class="center_img">
                    <? if (is_array($arResult["DETAIL_PICTURE"])){ ?>
                        <a href="<?=$arFileTmp["src"] /*$arResult["DETAIL_PICTURE"]["SRC"]*/ ?>" data-lightbox="roadtrip"><img class="class_img_item_0620e44395b9ad45e489d15b3615c972 zoom_0620e44395b9ad45e489d15b3615c972" alt="<?= $arResult["NAME"] ?>" src="<?=$arFileTmp["src"] /*$arResult["DETAIL_PICTURE"]["SRC"]*/ ?>" /></a>
                    <? }elseif (is_array($arResult["PREVIEW_PICTURE"])){ ?>
                        <a href="<?=$arFileTmp["src"] /*$arResult["PREVIEW_PICTURE"]["SRC"]*/ ?>" data-lightbox="roadtrip"><img class="class_img_item_0620e44395b9ad45e489d15b3615c972 zoom_0620e44395b9ad45e489d15b3615c972" alt="<?= $arResult["NAME"] ?>" src="<?=$arFileTmp["src"] /*$arResult["PREVIEW_PICTURE"]["SRC"]*/ ?>" /></a>
                    <? }else{ ?>
                    Нет фото
                    <? } ?>
                </div>
            ...
            <? elseif (isset($arResult["MORE_PHOTO"]) && (count($arResult["MORE_PHOTO"]) > 0)): ?>
                <div id="my-folio-of-works" class="svwp">
                    <ul>
                        <? if (is_array($arResult["DETAIL_PICTURE"])): ?>
                            <li><a href="<?=$arFileTmp["src"] /*$arResult["DETAIL_PICTURE"]["SRC"]*/ ?>" data-lightbox="roadtrip"><img class="class_img_item_0620e44395b9ad45e489d15b3615c972 zoom_0620e44395b9ad45e489d15b3615c972" alt="<?= $arResult["NAME"] ?>" src="<?=$arFileTmp["src"] /*$arResult["DETAIL_PICTURE"]["SRC"]*/ ?>" /></a></li>
                        <? elseif (is_array($arResult["PREVIEW_PICTURE"])): ?>
                            <li><a href="<?=$arFileTmp["src"] /*$arResult["PREVIEW_PICTURE"]["SRC"]*/ ?>" data-lightbox="roadtrip"><img class="class_img_item_0620e44395b9ad45e489d15b3615c972 zoom_0620e44395b9ad45e489d15b3615c972" alt="<?= $arResult["NAME"] ?>" src="<?=$arFileTmp["src"] /*$arResult["PREVIEW_PICTURE"]["SRC"]*/ ?>" /></a></li>
                        <? endif ?>

                        <? foreach ($arResult["MORE_PHOTO"] as $photo): ?>
                            <?
                            $arFileTmp_mini = CFile::ResizeImageGet(
                                $photo["ID"],
                                array("width" => 'auto', "height" => 'auto'),
                                BX_RESIZE_IMAGE_EXACT,
                                true,
                            ...
                            ?>
                            <li><a href="<?=$arFileTmp_mini["src"] /*$photo["SRC"]*/ ?>" data-lightbox="roadtrip"><img class="zoom_0620e44395b9ad45e489d15b3615c972" alt="<?= $arResult["NAME"] ?>" src="<?=$arFileTmp_mini["src"] /*$photo["SRC"]*/ ?>" /></a></li>
                        <? endforeach; ?>
                    </ul>
                </div>
            <? endif; ?>
        ...
        <div class="prices">
            <ul class="pricelist">
                ...
                <? if (is_array($arResult["PRICES"]["PRICE_WHS_1"]) && (count($arResult["PRICES"]["PRICE_WHS_1"]) > 0)): ?>
                    <li>
                        <span><?= $arResult["CAT_PRICES"]["PRICE_WHS_1"]["TITLE"] ?> при заказе от 10 000 руб.</span>
                        <span class="price">
                ...
                <? if (is_array($arResult["PRICES"]["PRICE_WHS_2"]) && (count($arResult["PRICES"]["PRICE_WHS_2"]) > 0)): ?>
                    <li>
                        <span><?= $arResult["CAT_PRICES"]["PRICE_WHS_2"]["TITLE"] ?> при заказе от 30 000 руб.</span>
                        <span class="price">
                ...
                <? if (is_array($arResult["PRICES"]["PRICE_WHS_3"]) && (count($arResult["PRICES"]["PRICE_WHS_3"]) > 0)): ?>
                    <li>
                        <span><?= $arResult["CAT_PRICES"]["PRICE_WHS_3"]["TITLE"] ?> при заказе от 80 000 руб.</span>
                        <span class="price">
            ...
            <div class="clear"></div>
			   <?
				...
				$ordStr = <<<STRCATWHOLESALE
									<div class="prior_to_joining">*до перехода в $replaceOPT осталось $relacePrice руб.</div>
STRCATWHOLESALE;
			//echo $ordStr;
    ...
    <div class="property">
        <div class="wrapp">
	<?	//echo "<pre>"; print_r($arResult["DISPLAY_PROPERTIES"]["FILES"]); echo "</pre>";?>
            <ul class="property_list">
                <?
                if (array_key_exists("DISPLAY_PROPERTIES", $arResult) && is_array($arResult["DISPLAY_PROPERTIES"])) {
                    foreach ($arResult["DISPLAY_PROPERTIES"] as $k => $property) {
                        if (!($property["DISPLAY_VALUE"] == "") && !is_array($property["DISPLAY_VALUE"])) {
                            ...
                            foreach ($property["DISPLAY_VALUE"] as $_k=> $valueProperty) {
                                if($k=="FILES") {
									?></br><a href="<?=$property["FILE_VALUE"][$_k]['SRC'];?>"><?=$property["FILE_VALUE"][$_k]['FILE_NAME'];?></a><?
								}
								else {
